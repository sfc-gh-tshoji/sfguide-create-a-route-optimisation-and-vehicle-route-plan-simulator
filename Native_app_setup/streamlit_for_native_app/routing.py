...
# industry lookup
lookup = session.table('DEFAULT_SCHEMA.LOOKUP')
...
st.markdown('''<h0black>ROUTE OPTIMISATION |</h0black><h0blue> SIMULATION</h0blue><BR>
<h1grey>Powered by the Open Route Service Native App</h1grey>''', unsafe_allow_html=True)
...
places_f = session.table('DEFAULT_SCHEMA.places')
...
with st.sidebar:
    ...
    st.info(prompt)
    model = st.selectbox('Choose Model:',['reka-flash','mistral-large2','claude-4-sonnet'],2)
    place = st.text_input('Choose Input','the Moscone Center in San Francisco')
    distance = st.number_input('Distance from location in KM',1,300,15)
...
time_slots = session.table('DEFAULT_SCHEMA.JOB_TEMPLATE')
...
# retrieve all potential distributors within specified point
   ...
   "style": {
    ...
    st.markdown(f'<h1sub>Wholesaler - {address.NAME.iloc[0]}</h1sub>',unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        based_on = st.selectbox('Based On:',method)

isochrone = session.create_dataframe([{'LON':start_1[0], 'LAT':start_1[1], 'METHOD': based_on,'RANGE_MINS':range_minutes}])
        #st.write(isochrone)
        
isochrone = isochrone.select(call_function('OPENROUTESERVICE_NATIVE_APP.CORE.ISOCHRONES',
...
data_for_chosen_point = [
    {"lon": start_1[0], "lat": start_1[1],'NAME': s_warehouse, 'DISTANCE': 0}
]
...
point_mark_layer = pdk.Layer(
    "ScatterplotLayer",
    places_vehicles.to_pandas(),
    get_position=["LON", "LAT"], # Tells PyDeck to use 'lon' and 'lat' for placement
    get_fill_color=[255, 0, 0, 200], # A nice red color for your mark
    get_radius=300,               # Make the mark 150 meters in radius for good visibility
    pickable=True,                # You can click on it!
    auto_highlight=True,          # It'll glow when you hover over it
)


view_state = pdk.ViewState(start_1[0],start_1[1], zoom=10)
...
with st.sidebar:
    ...
    st.pydeck_chart(pdk.Deck(layers=[isochrone,point_mark_layer],map_style=None,initial_view_state=view_state, tooltip=tooltip))
...
if submit:
    ...
    #try:
        ...
        optim = jobs.join(vehs).select('JOB','VEH',call_function('OPENROUTESERVICE_NATIVE_APP.CORE.OPTIMIZATION',
        ...
        .select('VEHICLE','R','G','B','PROFILE','ID','TOTAL_JOBS',
            call_function('OPENROUTESERVICE_NATIVE_APP.CORE.DIRECTIONS',
        ...
        optim_route_geometry =  optim.select('VEHICLE',col('GEOMETRY').alias('GEO'))
        optim_route_geometry = optim_route_geometry.join(vehsdet,vehsdet['ID']==optim_route_geometry['VEHICLE'])
        optim_route_geometry = optim_route_geometry.with_column('GEO',object_construct(lit('coordinates'),col('GEO')))
        optim_route_geometry = optim_route_geometry.with_column('NAME',concat(lit('Vehicle '),col('VEHICLE'),lit(' '),col('PROFILE')))
        #st.write(optim_route_geometry)
        ...
        data3 = optim_route_geometry.select('GEO','PROFILE','NAME','VEHICLE','ID','R','G','B').to_pandas()
        ...
        data3["coordinates"] = data3["coordinates"].apply(
       ...
       "text-wrap": "balance"
        ...
        with tab2:
            ...
            for a in d1.select('SEQ').distinct().order_by('SEQ').to_pandas().SEQ:
                job = job+1
                st.markdown(f'''<h1sub>Segment{job}</h1sub>''', unsafe_allow_html=True)
